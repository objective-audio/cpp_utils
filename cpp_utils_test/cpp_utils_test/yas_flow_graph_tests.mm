...
- (void)test_graph2 {
    ...
    graph.add_state(test_state::a, [](int const &signal) { return flow::wait(test_state::b); });
    graph.add_state(test_state::b, [](int const &signal) { return flow::run(test_state::c); });
    graph.add_state(test_state::c, [](int const &signal) { return flow::wait(test_state::a); });
